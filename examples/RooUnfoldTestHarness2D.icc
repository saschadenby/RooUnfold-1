...
void RooUnfoldTestHarness2D::Results()
{
  ...
  if (hUnfoldX) hUnfoldX->Draw("SAME");
  ...
  if (hUnfoldY) hUnfoldY->Draw("SAME");
    ...
    for (Int_t ix= 2; ix<=ntx-1; ix++) {
      for (Int_t iy= 2; iy<=nty-1; iy++) {
        if ( hUnfold->GetBinContent(ix,iy)!=0.0             && hUnfold->GetBinError(ix,iy)>0.0 &&
          ...
          Double_t pull= (hUnfold->GetBinContent(ix,iy) - hTrue->GetBinContent(ix,iy)) / hUnfold->GetBinError(ix,iy);
...
void RooUnfoldTestHarness2D::Reset()
{
  ...
  hTrainX= hTrainTrueX= hTrueX= hTrainFakeX= hFakeX= hMeasX= hUnfoldX= hPullsX=
  hTrainY= hTrainTrueY= hTrueY= hTrainFakeY= hFakeY= hMeasY= hUnfoldY= hPullsY= hPDFy= hTestPDFy= 0;
  dim= 2;
}
