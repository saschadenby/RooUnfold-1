...
void RooUnfoldTestHarness3D::Results()
{
  ...
  if (hUnfoldZ) hUnfoldZ->Draw("SAME");
  lTest->Draw();
  canvas->Update();
    ...
    for (Int_t iy= 1; iy<=nty; iy++) {
      for (Int_t iz= 1; iz<=ntz; iz++) {
        if ( hUnfold->GetBinContent(ix,iy,iz)!=0.0             && hUnfold->GetBinError(ix,iy,iz)>0.0 &&
          ...
          Double_t pull= (hUnfold->GetBinContent(ix,iy,iz) - hTrue->GetBinContent(ix,iy,iz)) / hUnfold->GetBinError(ix,iy,iz);
  ...
  if (onepage) canvas->cd(++ipad);
  hTrueXY->Draw();
  if (hUnfoldXY) hUnfoldXY->Draw("SAME");
  canvas->Update();
  ...
  if (onepage) canvas->cd(++ipad);
  hTrueYZ->Draw();
  if (hUnfoldYZ) hUnfoldYZ->Draw("SAME");
  canvas->Update();
  ...
  if (onepage) canvas->cd(++ipad);
  hTrueXZ->Draw();
  if (hUnfoldXZ) hUnfoldXZ->Draw("SAME");
  canvas->Update();
