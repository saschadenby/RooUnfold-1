...
TUnfold::TUnfold(const TMatrixD *matrix_A, EHistMap histmap, ERegMode regmode,
                 EConstraint constraint)
{
   // data members initialized to something different from zero:
   //    fA: filled from hist_A
...
Int_t TUnfold::SetInput(const TH1 *hist_y, Double_t scaleBias,
                        ...
                        const TH2 *hist_vyy_inv)
{
  ...
  for (Int_t iy = 0; iy < GetNy(); iy++) {
     ...
     Double_t dy2;
     if(!hist_vyy) {
        Double_t dy = hist_y->GetBinError(iy + 1);
        dy2=dy*dy;
        if (dy2 <= 0.0) {
  ...
  for (Int_t i = 0; i < GetNy(); i++) {
     (*fY) (i, 0) = hist_y->GetBinContent(i + 1);
  }
...
Int_t TUnfold::SetInput(const TVectorD *vec_y, const TVectorD *errvec_y, Double_t scaleBias,
                        Double_t oneOverZeroError,const TH2 *hist_vyy,
                        const TH2 *hist_vyy_inv)
{
  DeleteMatrix(&fVyyInv);
